...
If another transaction got there first, zero rows are affected and Ekbatan throws `StaleRecordException`. The action's transaction unwinds. The default `ExecutionConfiguration` retries the whole action once with a 100ms delay (tunable — see [Actions](/learn/actions/#retries-on-optimistic-lock-conflicts)).
...
<LearnPager current="models-and-entities" />

## See also

- [Actions, ActionPlan, ActionExecutor](/learn/actions/) — how Models and Entities flow through `perform()` into the database
- [Repositories on JOOQ](/reference/repository/) — how a Model becomes a row and back
- [The outbox: atomic state + events](/learn/outbox/) — where the events emitted by Models end up
