...
The exception is post-commit chaining: a local-event-handler `EventHandler`, broker consumer, or worker can react after the source action has committed and start the next action or service step. This is the shape used by [sagas](/learn/sagas/): one committed action emits an event, and a post-commit consumer starts the next step in its own transaction.
