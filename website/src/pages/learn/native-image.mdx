...
|---|---|
| Spring Boot | `spring-boot-starter-flyway` stays on the classpath, `spring.flyway.enabled=false` disables Boot's single-datasource auto-run, and a startup bean calls `FlywayMigrator.migrate(shardingConfig)`. |
