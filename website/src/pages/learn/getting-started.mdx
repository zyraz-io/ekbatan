...
Note what's **NOT** here: no `spring.datasource.*`, no `datasources.default`, no `quarkus.datasource.*`. The framework owns its pools directly; your stack's datasource auto-config is intentionally **not used** -- so Flyway and Ekbatan don't argue over connection coords. Step 9 runs Flyway from the same `ekbatan.sharding.*` block.

<div data-stack="spring" data-db="pg">

<CodeBlock filename="src/main/resources/application.yml" language="yaml" shadow="orange" code={`spring:
  autoconfigure:
...
Because step 8 sets `spring.flyway.enabled=false`, Spring Boot will not run its single-datasource Flyway initializer. This bean runs Flyway against every Ekbatan shard first, then adds an explicit dependency so Ekbatan's distributed job registry cannot start before migrations complete.
