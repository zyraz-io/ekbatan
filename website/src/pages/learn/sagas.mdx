...
title: "Sagas: chaining committed actions"
activeSection: learn
description: "How to model cross-shard or multi-step workflows as committed actions chained by outbox events, with forward-only compensation."
lead: "A saga is a way to model one business workflow as a sequence of smaller, independently committed actions. Use it when the workflow cannot honestly fit inside one database transaction."
---
...
The runnable example is [`ekbatan-examples/spring-boot-wallet-saga-gradle-pg`](https://github.com/ekbatan-io/ekbatan/tree/main/ekbatan-examples/spring-boot-wallet-saga-gradle-pg).
...
<LearnPager current="sagas" />

## See also

- [Listen-to-yourself: in-process event handlers](/reference/events/local-event-handler/) - the local handler pipeline used by the example
- [Sharding strategies](/learn/sharding-strategies/) - why cross-shard actions are rejected by default
- [The outbox: atomic state + events](/learn/outbox/) - why each saga step gets a durable event
- [Actions, ActionPlan, ActionExecutor](/learn/actions/) - why actions are not nested inside `perform()`
