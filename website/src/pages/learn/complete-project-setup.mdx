...
activeSection: learn
description: "The detailed setup matrix: scaffold, start a database, wire dependencies, write a model and repository, generate jOOQ classes, configure Flyway and run."
lead: "The full project wiring guide. Pick stack, build tool, and database once with the picker below; every setup snippet adapts."
---
...
If you are still learning the framework's model, start with the [Getting started](/learn/getting-started/) first. This page is the detailed setup checklist for wiring a real project once the core ideas make sense.
...
quarkus create app io.example:wallet:1.0.0-SNAPSHOT \
...
Add Ekbatan, `ekbatan-flyway`, your JDBC driver, and your stack's Flyway integration. Use the snippet as-is for your selected stack, build tool, and database; the short notes after the snippets explain the few non-obvious lines.

<div data-stack="spring" data-build="gradle" data-db="pg">

<CodeBlock filename="build.gradle.kts" language="kotlin" shadow="yellow" code={`// At the top of build.gradle.kts, outside dependencies.
...
<CodeBlock filename="build.gradle.kts" language="kotlin" shadow="orange" code={`// At the top of build.gradle.kts, outside dependencies.
extra["jooq.version"] = "3.20.10"
...
<CodeBlock filename="pom.xml" language="xml" shadow="default" code={`<!-- Inside <properties>. -->
...
<CodeBlock filename="build.gradle.kts" language="kotlin" shadow="yellow" code={`dependencies {
    ...
    compileOnly("io.github.zyraz-io:ekbatan-annotation-processor:1.0.0-RC1")
    annotationProcessor("io.github.zyraz-io:ekbatan-annotation-processor:1.0.0-RC1")
    annotationProcessor("io.github.zyraz-io:ekbatan-micronaut:1.0.0-RC1")  // required for Micronaut DI
...
<CodeBlock filename="build.gradle.kts" language="kotlin" shadow="yellow" code={`dependencies {
    ...
    jooqCodegen("org.mariadb.jdbc:mariadb-java-client:3.4.1")
    jooqCodegen("org.flywaydb:flyway-mysql:12.0.0")
}`} />
...
**Keep the jOOQ version override.** Spring Boot's BOM currently manages an older jOOQ line than Ekbatan uses. Put the override in the BOM property location shown above: `extra["jooq.version"]` for Gradle, `<jooq.version>` for Maven.

</div>

<div data-stack="quarkus">

**Keep the jOOQ version override.** Quarkus's BOM currently manages an older jOOQ line than Ekbatan uses. Put the override in the BOM property location shown above: `extra["jooq.version"]` for Gradle, `<jooq.version>` for Maven.

</div>

<div data-stack="micronaut">

**Keep the Micronaut annotation processor.** The extra `annotationProcessor("io.github.zyraz-io:ekbatan-micronaut:...")` line makes `@Ekbatan*` classes visible to Micronaut DI.

</div>

<div data-build="gradle">

**Keep the `jooqCodegen(...)` lines.** Gradle code generation runs on its own classpath, separate from the app runtime classpath. MariaDB and MySQL also include `flyway-mysql` there because the codegen migration step uses Flyway.

</div>

<div data-build="maven">

**Keep the codegen plugin dependencies.** Maven code generation runs on the plugin classpath, separate from the project dependencies. MariaDB and MySQL also include `flyway-mysql` there because the codegen migration step uses Flyway.

</div>

**Keep the Flyway dialect module.** Flyway core is not enough by itself. PostgreSQL uses `flyway-database-postgresql`; MariaDB and MySQL use `flyway-mysql`.
...
<CodeBlock filename="src/main/java/io/example/wallet/model/Wallet.java" language="java" shadow="yellow" errorTokens={["WalletBuilder"]} code={`package io.example.wallet.model;
...
public final class Wallet extends Model<Wallet, Id<Wallet>, WalletState> {
    ...
    Wallet(WalletBuilder builder) {
        ...
        this.balance  = Objects.requireNonNull(builder.balance,  "balance cannot be null");
    }
...
<CodeBlock filename="src/main/java/io/example/wallet/model/Wallet.java" language="java" shadow="yellow" code={`package io.example.wallet.model;
...
<CodeBlock filename="src/main/java/io/example/wallet/repository/WalletRepository.java" language="java" shadow="orange" errorTokens={["WALLETS", "WalletsRecord", "Wallets"]} code={`package io.example.wallet.repository;
...
At startup, the small migrator class later in this guide calls Flyway for every `V*.sql` in `src/main/resources/db/migration/` — in numeric order — and the jOOQ codegen plugin (step 6) replays the same migrations against a throwaway container to discover the schema. **Ekbatan does NOT bundle V0001/V0002 inside its jars** — you write them, or copy verbatim from any `ekbatan-examples/*-{pg,mariadb,mysql}/src/main/resources/db/migration/`.
...
At startup, the migrator calls Flyway and runs them in order: V0000 (MariaDB/MySQL only) → V0001 (eventlog) → V0002 (scheduled_tasks) → V0003 (wallets). The same set is what jOOQ codegen (step 6) reads against its throwaway container — so what your code references and what the database has are guaranteed to match.
## 6. Wire jOOQ codegen
    ...
    generateJooqClasses {
        ...
        usingJavaConfig {
            ...
            database.withForcedTypes(
                ...
                // MariaDB reports JSON columns as LONGTEXT, so codegen sees CLOB: withName rewrites
                // the generated type back to JSON so the converter attaches, and the column is
                // matched by name because no type regex can match "JSON" here.
                ForcedType()
                    .withName("JSON")
                    .withUserType("tools.jackson.databind.node.ObjectNode")
...
<plugin>
    ...
    <!-- Codegen plugin dependencies: driver plus Flyway dialect module for MariaDB/MySQL. -->
    <dependencies>
        <dependency>
            <groupId>org.postgresql</groupId>
            <artifactId>postgresql</artifactId>
                ...
                <generator>
                    <database>
                        ...
                        <forcedTypes>
                            ...
                            <!-- MariaDB reports JSON as LONGTEXT, so codegen sees CLOB.
                                 <name>JSON</name> rewrites the generated type back so the
                                 converter attaches; the column is matched by name because the
                                 reported type is not "JSON". -->
                            <forcedType>
                                <name>JSON</name>
                                <userType>tools.jackson.databind.node.ObjectNode</userType>
...
<CodeBlock filename="src/main/java/io/example/wallet/repository/WalletRepository.java" language="java" shadow="yellow" code={`package io.example.wallet.repository;
...
Note what's **NOT** here: no `spring.datasource.*`, no `datasources.default`, no `quarkus.datasource.*`. The framework owns its pools directly. The Spring Boot starter filters Boot's default single-datasource DataSource/Flyway auto-configuration, and the Quarkus/Micronaut examples do not configure a framework datasource. Step 9 runs Flyway from the same `ekbatan.sharding.*` block.
...
## 9. Run Flyway from the Ekbatan sharding config

Ekbatan already has the database connection coordinates under <code class="font-mono text-xs bg-yellow px-1">ekbatan.sharding.*</code>. Use `FlywayMigrator` from `ekbatan-flyway` to run migrations from that same config instead of copying the same JDBC URL, username, and password into a second framework-specific datasource block.

This is the same shape for one database or many databases. In this project setup page there is one configured shard, so `FlywayMigrator.migrate(shardingConfig)` runs Flyway once. In a sharded app, the same call runs the same migrations against every configured shard member's `primary-config`, sequentially and fail-fast. By default it uses `classpath:db/migration`; pass locations explicitly if you keep migrations elsewhere.

Spring Boot, Quarkus, and Micronaut still keep their Flyway integration dependencies from step 3. Those dependencies provide Flyway itself, the database-specific Flyway module, and native-image/framework integration where the stack needs it. The actual target selection is done by the small startup class below, using Ekbatan's typed `ShardingConfig`.

<div data-stack="spring">

The Ekbatan Spring Boot starter filters Spring Boot's default single-datasource DataSource/Flyway auto-configuration, so you do not need `spring.datasource.*`, `spring.autoconfigure.exclude`, or `spring.flyway.enabled=false`. This bean runs Flyway against every Ekbatan shard first, then adds an explicit dependency so Ekbatan's distributed job registry cannot start before migrations complete.

Escape hatch: set `ekbatan.spring.allow-boot-database-auto-configuration=true` only if you intentionally want Spring Boot to create or migrate its own separate conventional datasource. In normal Ekbatan applications, leave it unset.

<CodeBlock filename="src/main/java/io/example/wallet/startup/EkbatanShardFlywayMigrator.java" language="java" shadow="yellow" code={`package io.example.wallet.startup;
...
`FlywayMigration` is just a marker bean. Creating it runs the migration, and its bean name gives Spring something concrete that `ekbatanJobRegistry` can depend on before jobs start polling.

</div>

<div data-stack="quarkus">

Quarkus's own Flyway startup runner is not the migration owner in this setup. The observer below owns migration startup and runs Flyway against every Ekbatan shard before the rest of the application starts.

<CodeBlock filename="src/main/java/io/example/wallet/startup/EkbatanShardFlywayMigrator.java" language="java" shadow="orange" code={`package io.example.wallet.startup;
...
`@Context` makes Micronaut instantiate this bean eagerly at startup, before lazy `@Singleton` beans such as Ekbatan's `DatabaseRegistry` touch the database. The constructor runs migrations synchronously from `ShardingConfig`.

<CodeBlock filename="src/main/java/io/example/wallet/startup/EkbatanFlywayMigrator.java" language="java" shadow="yellow" code={`package io.example.wallet.startup;
...
No DI container, no startup hook. Call `FlywayMigrator.migrate(shardingConfig)` before you build the `DatabaseRegistry` and repositories. See [Plain Java wiring](/reference/di/without-di/) for the full bootstrap.
...
If you see the eventlog tables, Flyway ran the migration files in `src/main/resources/db/migration`, including the Ekbatan eventlog schema, and the app is fully wired.

## 14. Use the example projects as your reference

Every snippet on this page came from [`ekbatan-examples/`](https://github.com/zyraz-io/ekbatan/tree/main/ekbatan-examples) — 38 runnable wallet REST projects covering Spring Boot, Quarkus, and Micronaut, each on PostgreSQL/MariaDB/MySQL, in Gradle and Maven, with native-image siblings and sharded Spring Boot variants. The picker's **EXAMPLE** row links straight to the matching framework example; plain Java points to the manual wiring reference.
...
- `spring-boot-wallet-rest-gradle-native-sharded-pg` — the same sharded app compiled and tested as a native image
...
<LearnPager current="complete-project-setup" />

## See also

- [Learn → The dual-write trap](/learn/the-dual-write-trap/) — why the eventlog table exists
- [Reference → DI integration](/reference/) — exhaustive per-stack wiring
- [`ekbatan-examples/`](https://github.com/zyraz-io/ekbatan/tree/main/ekbatan-examples) — 38 runnable framework wallet REST projects, plus saga and job-worker examples
