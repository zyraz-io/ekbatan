...
activeSection: learn
lead: 'After your first Action commits, eventlog.events contains a durable event row. From there you choose how to use it: handle it locally, publish from a handler, stream with CDC, or fan out through a router.'
---
...
Per the [listen-to-yourself pattern](/reference/events/local-event-handler/), the handler can dispatch its own actions via an injected `ActionExecutor`, and those actions persist their own resulting events through the normal action flow. That same shape is used for [sagas](/learn/sagas/) when one committed action should trigger the next committed action.
...
```json
{
  ...
  "config": {
    ...
    "transforms.encodeAvro.action.event.schema": "/schemas/ActionEvent.avsc",
    "transforms.encodeAvro.payload.schemas": "com.example.finance.avro.WalletMoneyDepositedEvent:/schemas/WalletMoneyDepositedEvent.avsc",
...
- Store the event row id, or a domain-specific idempotency key, in the consumer's table before applying the side effect.
...
- **CDC to a raw topic** turns committed event rows into a cross-service event stream without publishing from request handlers.
- **CDC plus downstream routing** lets one raw stream become many topics or differently keyed streams.
- All of them read from the same `eventlog.events` rows. The dual-write trap is structurally impossible — there is only one durable write inside the action's transaction, and everything else consumes from it.
...
<LearnPager current="consuming-events" />

## See also

- [Learn → The outbox: atomic state + events](/learn/outbox/) — why the durable outbox exists
...
- [Reference -> Framework tables](/reference/tables/) — table map plus full DDL for `eventlog.events` and `eventlog.event_notifications`
