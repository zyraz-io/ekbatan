...
title: 'Listen-to-yourself: in-process event handlers'
activeSection: reference
lead: 'For applications that don''t need to fan events out to separate services — small monoliths, internal tools, or anywhere a Kafka cluster would be overkill — the <code>ekbatan-events:local-event-handler</code> module consumes the same <code>eventlog.events</code> outbox <strong>inside the same JVM</strong> via two background jobs. Same outbox row, same atomic-with-the-action guarantee, no broker.'
---

import AliasRenameFlow from '../../../components/AliasRenameFlow.astro';
...
While that version was running, fan-out created rows with `handler_name = 'widget-created-email'`.
...
<AliasRenameFlow
    oldName="widget-created-email"
    newName="widget-created-welcome-email"
    handlerClass="WidgetCreatedWelcomeEmailHandler"
/>
...
The `delivered` column on `eventlog.events` is part of the base outbox schema (see [outbox-schema.md](/reference/outbox-schema/)) — every Ekbatan deployment already has it, written as `FALSE` on insert. The local-event-handler path adds an `events_undelivered` partial index for the fan-out scan and a new `eventlog.event_notifications` table. PostgreSQL:
...
For MySQL/MariaDB equivalents, drop the `WHERE` clauses on the partial indexes and use the dialect-appropriate UUID/JSON column types — see [Multi-database](/reference/multi-database/) and the migrations under [<code>ekbatan-integration-tests/local-event-handler/&#123;mysql,mariadb&#125;/<wbr />src/test/resources/db/migration/</code>](../../ekbatan-integration-tests/local-event-handler/).
...
- [The outbox: atomic state + events](/concepts/outbox/) — the upstream source of events
- [Distributed background jobs](/reference/jobs/distributed-jobs/) — `EventFanoutJob` and `EventHandlingJob` are themselves `DistributedJob`s
- [Wiring with Spring Boot](/reference/di/spring-boot/) / [Quarkus](/reference/di/quarkus/) / [Micronaut](/reference/di/micronaut/) — `@EkbatanEventHandler` discovery
- [Streaming via Debezium → Kafka](/reference/events/streaming/) — the alternative consumer path
