...
See [`ekbatan-examples/quarkus-wallet-rest-gradle-pg`](https://github.com/ekbatan-io/ekbatan/tree/main/ekbatan-examples/quarkus-wallet-rest-gradle-pg) for the full runnable shape (and its `-mariadb`/`-mysql`/`-native-*`/`-maven-*` variants for the other dialects, build tools, and native).
