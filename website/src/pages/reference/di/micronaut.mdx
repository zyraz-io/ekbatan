...
See [`ekbatan-examples/micronaut-wallet-rest-gradle-pg`](https://github.com/ekbatan-io/ekbatan/tree/main/ekbatan-examples/micronaut-wallet-rest-gradle-pg) for the full runnable shape (and its `-mariadb`/`-mysql`/`-native-*`/`-maven-*` variants).
