...
lead: 'What you write in a Micronaut app to use Ekbatan: the integration jar plus the framework''s compile-time annotation processor, four <code>@Ekbatan*</code> annotations on your own classes, and an <code>application.yml</code> tree under <code>ekbatan.*</code>. The compile-time <code>EkbatanStereotypeVisitor</code> lifts your annotated classes to <code>@Singleton</code> so Micronaut generates <code>BeanDefinition</code>s for them, and <code>ActionExecutor</code> is injectable anywhere.'
---

For the equivalent in plain Java with no DI container, see [wiring/without-di.md](/reference/di/without-di/). For Spring Boot and Quarkus, see [spring.md](/reference/di/spring-boot/) and [quarkus.md](/reference/di/quarkus/).
...
- **Native works with the same application code.** In a native image, `FlywayMigrator` installs an internal classpath resource scanner so migrations can still be discovered inside the binary. See [GraalVM native-image](/reference/runtime/native-image/) for the stack-by-stack details.
...
See [`ekbatan-examples/micronaut-wallet-rest-gradle-pg`](https://github.com/zyraz-io/ekbatan/tree/main/ekbatan-examples/micronaut-wallet-rest-gradle-pg) for the full runnable shape (and its `-mariadb`/`-mysql`/`-native-*`/`-maven-*` variants).
...
- [Wiring without DI](/reference/di/without-di/) — what the visitor + factory beans are doing for you
- [Wiring with Spring Boot](/reference/di/spring-boot/) / [Wiring with Quarkus](/reference/di/quarkus/) — same end state in the other DI frameworks
...
- [GraalVM native-image](/reference/runtime/native-image/) — Micronaut + native specifics
