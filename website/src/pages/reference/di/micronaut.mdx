...
- **Native works with one small change.** The JVM examples call `Flyway.configure()...migrate()` directly. The native-image examples keep the same eager `@Context` migrator shape, but the migrator delegates to `FlywayHelper.migrate(...)` so classpath migrations can be discovered inside the native image. See [GraalVM native-image](/reference/runtime/native-image/) for the stack-by-stack details.
