...
- **Native works.** Keeping `micronaut-flyway` on the classpath preserves its substrate-VM Flyway scanning hints. For the native-image variants of the example projects, the migrator delegates to `FlywayHelper.migrate(...)` instead — see [GraalVM native-image](/reference/runtime/native-image/) for why.
