...
| 3. TM + registry | One `TransactionManager` per shard; `DatabaseRegistry` indexes them by `ShardIdentifier` | [The outbox](/learn/outbox/), [Sharding](/reference/sharding/) |
...
| 5. Actions | One `Action` instance per concrete subclass — they're singletons, per-call state lives on the plan | [Actions](/learn/actions/) |
...
| 7. `ActionExecutor` | The framework's central entry point — wraps every call in one transaction | [Actions](/learn/actions/), [The outbox](/learn/outbox/) |
...
- [Actions](/learn/actions/) — what `ActionExecutor` actually does when you call `execute(...)`
