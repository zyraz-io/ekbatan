...
7. Add the per-dialect codegen block to the new test module — [Gradle](/learn/getting-started/#jooq-codegen) (`build.gradle.kts`) or [Maven](/learn/getting-started/#jooq-codegen) (`pom.xml`).

## See also

- [JOOQ codegen](/reference/jooq-codegen/) — what codegen generates, the seven framework converters, per-dialect modeling rationale
- [JOOQ codegen on Gradle](/learn/getting-started/#jooq-codegen) — the per-dialect `build.gradle.kts` blocks
- [JOOQ codegen on Maven](/learn/getting-started/#jooq-codegen) — the equivalent `pom.xml` plugin chain
...
- [Outbox schema](/reference/outbox-schema/) — the on-disk shape of `eventlog.events` and friends
