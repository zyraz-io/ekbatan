...
MariaDB only - MySQL reports `JSON` correctly and keeps `withIncludeTypes("(?i:JSON)")`. See [JOOQ codegen on Gradle](/learn/complete-project-setup/#jooq-codegen) (for `generateJooqClasses { … }`) or [JOOQ codegen on Maven](/learn/complete-project-setup/#jooq-codegen) (for `<forcedType>`) for the full block.
