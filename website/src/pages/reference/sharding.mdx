...
Prefer the per-call override so the risk is visible at the call site. Use an executor default only for a dedicated executor whose actions are all designed for per-shard commits. If partial commits would require compensation, use a [saga](/learn/sagas/) instead of one cross-shard action.
...
- [Actions](/learn/actions/) — `allowCrossShard` is a per-call `ExecutionConfiguration` knob
- [Sagas: chaining committed actions](/learn/sagas/) — the compensation pattern for workflows that cannot tolerate hidden partial commits
- [The outbox: atomic state + events](/learn/outbox/) — how events are duplicated across shards in cross-shard mode
