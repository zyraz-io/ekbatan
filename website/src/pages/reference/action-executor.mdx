...
Treat this as an escape hatch. Prefer passing `allowCrossShard(true)` for one explicit `execute(...)` call, not enabling it as a broad executor default. If partial commits would need compensation, use a [saga](/concepts/sagas/) instead of one cross-shard action.
