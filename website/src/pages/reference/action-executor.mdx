...
Treat this as an escape hatch. Prefer passing `allowCrossShard(true)` for one explicit `execute(...)` call, not enabling it as a broad executor default. If partial commits would need compensation, use a [saga](/learn/sagas/) instead of one cross-shard action.

See [Learn → Sharding strategies → Cross-shard actions](/learn/sharding-strategies/#cross-shard-actions-one-transaction-per-shard-never-atomic-across) for the consistency model and the saga mitigation pattern.

## Retries

Each `execute(...)` call is wrapped in a `Retry` driver keyed on the configured `RetryConfig`s. Matching is exact by exception class and also checks the cause chain; superclass matching is not used. The default `ExecutionConfiguration` retries `StaleRecordException` **once after 100ms** — enough to absorb a transient optimistic-lock conflict without hiding a deeper problem. **A retry builds a brand-new `ActionPlan`** so each attempt is logically independent.
...
executor.eventPersister.persistActionEvents(
        namespace, sourceActionName, startedAt, completionAt, params, events, shard, actionId);
```
...
- [Learn → Actions, ActionPlan, ActionExecutor](/learn/actions/) — the conceptual two-phase lifecycle
...
- Source: [`ekbatan-core/src/main/java/io/ekbatan/core/action/ActionExecutor.java`](https://github.com/ekbatan-io/ekbatan/blob/main/ekbatan-core/src/main/java/io/ekbatan/core/action/ActionExecutor.java)
