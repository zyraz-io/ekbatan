...
The retry map can contain multiple exception classes. Exceptions not present in the effective retry config propagate to the caller.
...
The executor then opens one transaction per shard, in deterministic order, and rolls each back independently on failure. You can also set this as the executor default, but there is no action-level `@EkbatanAction` switch. See [Sharding](/reference/sharding/) for the consistency caveats.
