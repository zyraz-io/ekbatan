...
title: 'eventlog.events'
activeSection: reference
lead: 'The durable event table written with each action transaction.'
---
...
<CodeBlock filename="src/main/resources/db/migration/V0000__create_eventlog_database.sql" language="sql" shadow="yellow" code={`CREATE DATABASE IF NOT EXISTS eventlog;`} />
...
The local event handler adds one extra fan-out index on this table. That index is documented with [`eventlog.event_notifications`](/reference/tables/event-notifications/) because it is only needed when local event handling is installed.
...
- [The outbox: atomic state + events](/concepts/outbox/) — why this table exists.
- [Streaming via Debezium](/reference/events/streaming/) — reading committed event rows with CDC.
- [Local event handler](/reference/events/local-event-handler/) — fan-out from event rows into local handler notifications.
