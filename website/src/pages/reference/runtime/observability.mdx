...
title: 'OpenTelemetry tracing and metrics'
activeSection: reference
lead: 'Ekbatan instruments its action execution pipeline with the <strong>OpenTelemetry API</strong> (<code>opentelemetry-api</code>). The library depends only on the API — no SDK. When no OTel SDK is registered at runtime, all tracing calls are no-ops with zero overhead. Consumers bring their own <code>opentelemetry-sdk</code> and exporters.'
---
...
- [Actions, ActionPlan, ActionExecutor](/learn/actions/) — the source of `action.execute` / `action.perform` / `action.persist`
- [Sharding](/reference/sharding/) — `ekbatan.shard.*` attributes come from the per-shard `TransactionManager`
- [Repositories on JOOQ](/reference/repository/) — what the `ekbatan.repository` span wraps
