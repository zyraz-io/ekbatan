...
lead: 'Ekbatan instruments its action execution pipeline with the **OpenTelemetry API** (`opentelemetry-api`). The library depends only on the API — no SDK. When no OTel SDK is registered at runtime, all tracing calls are no-ops with zero overhead. Consumers bring their own `opentelemetry-sdk` and exporters.'
---
...
- [Actions, ActionPlan, ActionExecutor](/concepts/actions/) — the source of `action.execute` / `action.perform` / `action.persist`
