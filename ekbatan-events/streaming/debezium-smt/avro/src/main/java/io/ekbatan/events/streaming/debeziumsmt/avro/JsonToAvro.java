...
final class JsonToAvro {
    ...
    private Object logicalValue(JsonNode node, Schema schema, String path) {
        ...
        return switch (logical.getName()) {
            case "timestamp-millis" -> epoch(node, path, 1L);
            case "timestamp-micros" -> epoch(node, path, 1_000L);
    ...
    private static long epoch(JsonNode node, String path, long multiplier) {
        if (node.isNumber()) {
