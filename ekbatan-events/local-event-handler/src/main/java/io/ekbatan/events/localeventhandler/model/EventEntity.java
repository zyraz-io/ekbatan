...
/**
 * Immutable in-memory representation of a single {@code eventlog.events} row, mirroring the
 * shape of {@code SingleTableJsonEventPersister}'s package-private {@code EventEntity} in
 * {@code ekbatan-core} plus the {@code delivered} column this module adds to the schema.
 * Same fields, same builder-POJO style — duplicated here because the core class is
 * package-private and our module needs an equivalent type for cross-package use (persister,
 * fan-out, dispatch).
 *
 * <p>The {@code delivered} flag indicates whether the fan-out job has materialized
 * {@code event_notifications} rows for this event yet. Newly-persisted events default to
 * {@code false}; the fan-out job flips them to {@code true} after writing the corresponding
 * notification rows.
 */
public final class EventEntity {
    ...
    public boolean equals(Object other) {
        ...
        if (!(other instanceof EventEntity that)) return false;
