...
MariaDB stores `JSON` as `LONGTEXT` with a CHECK constraint internally, and the JDBC driver reports the type accordingly.

This affects **code generation only**. A hand-declared field such as `SQLDataType.JSON.asConvertedDataType(new JSONObjectNodeConverter())` tells jOOQ the type rather than asking the database, so it is unaffected. Codegen takes the reported type, which on MariaDB is `LONGTEXT` -> `CLOB` -> `DataType<String>`; matching on `(?i:JSON)` therefore never fires, and widening to `(?i:JSON|LONGTEXT)` matches but does not compile, because the shared converters are `Converter<JSON, ..>`.

Add `withName("JSON")` to rewrite the generated type back, and match by column expression:
...
MariaDB only - MySQL reports `JSON` correctly and keeps `withIncludeTypes("(?i:JSON)")`. See [JOOQ codegen on Gradle](../gradle/jooq-codegen.md) (for `generateJooqClasses { … }`) or [JOOQ codegen on Maven](../maven/jooq-codegen.md) (for `<forcedType>`) for the full block.
