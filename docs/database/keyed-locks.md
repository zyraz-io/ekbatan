...
`InProcessKeyedLockProvider` and `RedisKeyedLockProvider` block on primitives that respond to `Thread.interrupt()` directly. The three SQL providers can't: a single unbounded `pg_advisory_lock` / `GET_LOCK` call parks the thread inside a JDBC socket read, which no interrupt can break. So they wait in **5-second segments** and check for interruption between them.

Two consequences worth knowing:

- **Interrupt latency is up to 5 seconds.** Well inside typical 30-second graceful-shutdown budgets, but not instant.
- **Fairness is weaker under contention.** During each segment the waiter is genuinely queued inside the database, so hand-off from a releasing holder is immediate — this is not client-side spinning. But every new segment re-enters that queue at the *back*, so a long-waiting thread repeatedly yields position to newer arrivals. Only `InProcessKeyedLockProvider` promises FIFO; treat the SQL backends as best-effort, and prefer optimistic locking if you're contending hard enough for this to matter.
