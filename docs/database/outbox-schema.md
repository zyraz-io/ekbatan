...
CDC consumers should skip these by checking `event_type IS NULL`. The framework's bundled SMTs ([Streaming via Debezium → Kafka](../events/event-streaming.md)) already do this.

## Cross-dialect column types

The logical schema is the same on every supported database; the column types differ. Quick table — the [Multi-database](multi-database.md) doc has the full cheatsheet:
...
In MySQL/MariaDB, `eventlog` is a separate **database** (not a schema), so the first migration must `CREATE DATABASE IF NOT EXISTS eventlog;`. The named test user typically also needs cross-database `GRANT`s — see [Multi-database](multi-database.md#schema-vs-database).

All timestamps are UTC. Use `TIMESTAMP` (or `DATETIME(6)`), **never** `TIMESTAMPTZ`.

## When the local-event-handler is in use

The `delivered` flag is part of the base schema above — every Ekbatan deployment writes it as `FALSE` on every insert, regardless of whether the in-process consumer path is wired up. The [in-process consumer path](../events/local-event-handler.md) adds the partial index that the fan-out scan needs and a separate `event_notifications` table:
...
- [The outbox: atomic state + events](../concepts/outbox.md) — what this schema is *for*
- [Multi-database](multi-database.md) — full dialect cheatsheet, init scripts, partial-index notes
- [Sharding](sharding.md) — how the outbox lives on each shard (cross-shard events get one row per shard, same UUID)
- [Listen-to-yourself](../events/local-event-handler.md) — the consumer that drives the `delivered` flag and `event_notifications`
- [Streaming via Debezium → Kafka](../events/event-streaming.md) — the CDC consumer
