...
Ekbatan publishes 15 JARs to Maven Central and has no Gradle-only runtime requirement — every framework artifact works the same way in a Maven project. This category covers the build-tool surface that a Maven consumer actually has to write.
