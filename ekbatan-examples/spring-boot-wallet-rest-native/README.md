# spring-boot-wallet-rest-native

The GraalVM native-image variant of [`spring-boot-wallet-rest`](../spring-boot-wallet-rest). The domain code is identical — only the build configuration and one small AOT guard differ. Start with the JVM sibling if you're new to Ekbatan; come back here once you want a native binary.
...
- **`graalvmNative { ... }` block** — pins the toolchain to GraalVM 25 (auto-detected from SDKMAN/asdf/system installs via Gradle's `javaToolchains`), bundles `db/migration/*.sql` into the image, enables the published reachability-metadata repository, and tells `Jackson3RecordsFeature` to scan `io.example` (in addition to its default `io.ekbatan` scan root) so record components for our `Action.Params` records are reflectively registered.
...
You don't need to set `JAVA_HOME` — Gradle's toolchain auto-detection picks installed JDKs up. Then:

```bash
./gradlew nativeCompile             # produces build/native/nativeCompile/spring-boot-wallet-rest-native (~120 MB, ~2 minutes)
...
- [`spring-boot-wallet-rest`](../spring-boot-wallet-rest) — the JVM-only baseline.
