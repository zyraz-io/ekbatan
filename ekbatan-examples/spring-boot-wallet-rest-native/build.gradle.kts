...
plugins {
    ...
    // GraalVM native-image support. Provides `nativeCompile` (produces the native binary),
    // `nativeRun` (runs it), and `nativeTest` (compiles + runs the test suite as a native
    // image). The Spring Boot plugin auto-applies its AOT integration once this is on the
    // classpath.
...
dependencies {
    ...
    // ── Flyway ──────────────────────────────────────────────────────────────
    // We run Flyway programmatically via FlywayHelper from ekbatan-native (native-image-aware
    // wrapper around Flyway.configure(...).load().migrate()). It reads from Ekbatan's
    // ekbatan.sharding.* config, not spring.datasource.*, so there's a single source of truth.
