...
| [`spring-boot-wallet-rest-gradle-sharded-pg`](./spring-boot-wallet-rest-gradle-sharded-pg) | Spring Boot + **2× Postgres** | Sharding. `Wallet` uses `ShardedId`, `WalletRepository` uses `EmbeddedBitsShardingStrategy`, two Postgres containers stand in for two policy-axis shards (global + mexico). `WalletTransferAction` opts into `allowCrossShard(true)` and runs two transactions, one per shard. |
