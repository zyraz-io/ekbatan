...
| Flyway integration | **`spring-boot-starter-flyway`** + `@FlywayDataSource @Bean DataSource` from `ShardingConfig` | **`quarkus-flyway`** + `EkbatanShardFlywayCustomizer` implementing `FlywayConfigurationCustomizer` (CDI) | `micronaut-flyway` on the classpath + an eager `@Context` migrator that reads `ShardingConfig` directly |
...
> **Each wallet keeps one source of truth for Flyway connection settings.** Spring Boot uses `spring-boot-starter-flyway` with a `@FlywayDataSource` bean. Quarkus uses `quarkus-flyway` with an `EkbatanShardFlywayCustomizer`. Micronaut skips the auto-wired `flyway.datasources.default` block and runs an eager `@Context` migrator from the typed `ShardingConfig`. See the wiring docs for the exact dep coordinates and shape per framework: [Spring Boot](../docs/wiring/spring.md#flyway--use-spring-boot-starter-flyway--a-flywaydatasource-bean) · [Quarkus](../docs/wiring/quarkus.md#flyway--use-quarkus-flyway--a-flywayconfigurationcustomizer) · [Micronaut](../docs/wiring/micronaut.md#flyway--programmatic-context-bean).
>
> For native-image, Spring Boot and Quarkus keep their framework Flyway integrations. The Micronaut native examples use the same eager migrator shape but delegate the actual migration call to `FlywayHelper.migrate(...)`, because it is the native-aware path for programmatic Flyway. Plain Java apps and framework integration tests use `FlywayHelper` directly. See [docs/runtime/native-image.md](../docs/runtime/native-image.md#flyway-on-native).
