...
class WalletControllerIntegrationTest {
    ...
    void separate_wallets_route_to_their_own_shards() throws Exception {
        ...
        await().atMost(Duration.ofSeconds(15))
                ...
                .untilAsserted(() -> {
                    final var notifications = notificationRepository.findAllByWalletId(mexicoId);
                    assertThat(notifications).hasSize(1);
                    assertThat(notifications.getFirst().kind).isEqualTo(NotificationKind.MONEY_DEPOSITED);
                    assertThat(notifications.getFirst().recipient).isEqualTo("mexico@example.com");
                    assertThat(notifications.getFirst().message).contains("Deposit of");
                });
    }
