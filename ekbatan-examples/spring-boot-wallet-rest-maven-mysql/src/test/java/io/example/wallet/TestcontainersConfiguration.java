...
class TestcontainersConfiguration {
    ...
    DynamicPropertyRegistrar ekbatanShardingProperties(MySQLContainer mysql) {
        return registry -> {
            ...
            registry.add("ekbatan.sharding.groups[0].members[0].configs.lockConfig.jdbcUrl", mysql::getJdbcUrl);
            registry.add("ekbatan.sharding.groups[0].members[0].configs.lockConfig.username", mysql::getUsername);
            registry.add("ekbatan.sharding.groups[0].members[0].configs.lockConfig.password", mysql::getPassword);
            registry.add(
