# spring-boot-wallet-rest-gradle-mariadb
...
6. **No partial indexes.** Postgres uses `WHERE delivered = FALSE` for the events index to keep polling cheap; MariaDB doesn't support partial indexes, so we drop the `WHERE` clause and accept a slightly less selective index. The polling query still filters at the predicate level.
