...
// jOOQ codegen needs a running MariaDB container to introspect the live schema. The plugin spins
...
// of where the eventlog database lives — both are MariaDB, identical dialect.
jooq {
    withContainer {
        image {
            name = "mariadb:11.8"
...
dependencies {
    ...
    // EkbatanShardFlywayMigrator runs migrations over every shard from ekbatan.sharding.*. The
    ...
    // a newer driver.
    runtimeOnly("org.mariadb.jdbc:mariadb-java-client:$mariadbDriverVersion")
    ...
    //   org.testcontainers:mysql            → org.testcontainers:testcontainers-mariadb
...
tasks {
    generateJooqClasses {
        // MariaDB has no schemas (schema = database), so only the main 'wallet' DB is generated.
        ...
        usingJavaConfig {
            database.withForcedTypes(
                ...
                ForcedType()
                    ...
                    .withIncludeExpression(".*"),
                // No UUID forced type — MariaDB 10.7+ has a native UUID type that jOOQ
                // maps to java.util.UUID directly. Contrast with MySQL, which has no native
                // UUID and uses UuidStringConverter on CHAR(36).
            )
        }
    }
...
graalvmNative {
    ...
    binaries.all {
        ...
        resources.includedPatterns.add("mariadb_init\\.sql")
